  ...
  try {
    ...
    if (Object.keys(data).length == 0) {
      return res
        ...
        .send({
          status: false,
          msg: "plese enter some data in order to create",
        });
    } else if (name == undefined || name.trim() == "") {
      res.status(400).send({ status: false, msg: "plese enter your name" });
    } else if (!isValidFullName(name.trim())) {
      res
        .status(400)
        .send({
          status: false,
          msg: "plese enter a valid fullname of your college",
        });
      ...
      return res
        ...
        .send({
          status: false,
          msg: "plese enter a valid name of your college",
        });
      ...
      if (collegeFilter == null) {
        res
          ...
          .send({
            status: false,
            msg: `sorry we cant find college with ${collegeName} `,
          });
      } else {
  ...
  try {
    ...
    if (!isValideName(data)) {
      return res
        ...
        .send({
          status: false,
          msg: `college name should only consist of alphabets`,
        });
    }
    if (!data) {
      return res
        .status(400)
        .send({ status: false, msg: `plese enter collegeName  ` });
    }
    console.log(data);
    let getData = await collegeModel.findOne({
      name: { $eq: data },
      isDeleted: false,
    });
